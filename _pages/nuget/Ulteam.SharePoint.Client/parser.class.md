...
permalink: /nuget/Ulteam.SharePoint.Client/parser/
sidebar:
  nav: "nuget"
---
...
| Name | Description |
|-|-|
| GetFieldValue(ListItem Item, Field Field, ParseSetting Setting) | Получение текстового значения из Field в зависимости от типа поля.  <br> **Item**: *Элемент списка.*  <br> **Field**: *Поле для получения значения.*  <br> **Setting**: *Настройка парсинга.*  |
...
| ParseFieldTag(ClientContext Context, ListItem Item, String ItemType) | Парсинг тега Field.               Вместо тега подставляется значение поля (name) элемента списка (type) SharePoint.               Возможные атрибуты тега:                type (тип элемента списка),               name (internal name поля в списке),                format (формат поля типа дата и время),                separator (разделитель для полей MultiUser, MultiChoice, MultiLookup),                truevalue (значение, если в поле стоит true),                falsevalue (значение, если в поле стоит false).  <br> **Context**: *SharePoint site collection context.*  <br> **Item**: *Элемент списка, из которого будут браться значения.*  <br> **ItemType**: *Тип элемента списка, указанное в атрибуте type тега.  Если параметр не указан, обрабатываются теги без атрибута type, либо с type=''.*  |
| ParseFilledTag(ClientContext Context, ListItem Item, String ItemType) | Парсинг тега Filled.              Если значение в поле (name) элемента списка (type) не пустое, то вернется InnerXml тега.              Возможные атрибуты тега:               type (тип элемента списка),              name (internal name поля в списке).  <br> **Context**: *SharePoint site collection context.*  <br> **Item**: *Элемент списка, из которого будут браться значения.*  <br> **ItemType**: *Тип элемента списка, указанный в атрибуте type тега.  Если параметр не указан, обрабатываются теги без атрибута type, либо с type=''.*  |
| ParseFormUrlTag(ClientContext Context, ListItem Item, String ItemType) | Парсинг тега FormUrl.              Заменяется на <a href='{name}'>{InnerXml}</a>              Возможные атрибуты тега:               type (тип элемента списка),              name (internal name поля в списке).  <br> **Context**: *SharePoint site collection context.*  <br> **Item**: *Элемент списка, для которого находятся FormUrl.*  <br> **ItemType**: *Тип элемента списка, указанный в атрибуте type тега.  Если параметр не указан, обрабатываются теги без атрибута type, либо с type=''.*  |
| ParseHistoryTag(ClientContext Context, XmlNode Node, String NodeType) | Парсинг тега History.              Возможные атрибуты тега:               type (тип элемента списка),              name - название класса html элемента, из которого необходимо взять значение (InnerText).  <br> **Context**: *SharePoint site collection context.*  <br> **Node**: *Узел из HTML истории процесса с шагом, из которого необходимо взять данные.*  <br> **NodeType**: *Тип элемента, указанное в атрибуте type тега.  Если параметр не указан, обрабатываются теги без атрибута type, либо с type=''.*  |
...
public void ExampleParseFunction()
{
    ...
    ListItem mainItem = TestSettings.ParserListItems[0];
    ListItem taskItem = TestSettings.ParserListItems[1];
